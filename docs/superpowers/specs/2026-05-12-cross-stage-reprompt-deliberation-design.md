...
3. New canon policy `pol-cross-stage-reprompt-default`:
   ...
   **Severity filter interaction with `reprompt_target`**: the severity filter applies BEFORE target routing. A finding whose severity is not in `severities_to_reprompt` (e.g. `severity='major'` under the indie default of `['critical']`) is treated as a normal finding: the `reprompt_target` field is ignored for re-prompt routing, the finding flows through `pol-auditor-feedback-reprompt-default` like any other audit finding (which may or may not re-prompt the auditing stage depending on that policy's own severity filter), and the runner does NOT walk back to the upstream stage. The flow is: (1) auditor emits findings; (2) runner filters by `severities_to_reprompt`; (3) for findings that pass, runner partitions by `reprompt_target` (cross-stage vs intra-stage); (4) findings that fail the severity filter remain in the audit-trail atom for operator visibility but do not trigger re-runs. Org-ceiling deployments that want major-severity findings to drive cross-stage re-prompts widen `severities_to_reprompt` in the canon-policy atom.

   `allowed_targets` is DERIVED at runtime from the active pipeline composition rather than hardcoded so the spec stays valid for any stage shape. The runner exposes `pipelineComposition.allowedRepromptTargets()`: every stage in the composition EXCEPT the terminal stage (dispatch-stage in the 5-stage default) and any stage flagged as audit-only (review-stage in the 5-stage default; the `audit_only: true` flag on the PlanningStage interface is the seam) is an allowed target. Org-ceiling deployments that compose additional stages (legal-review, perf-benchmark) automatically get them as re-prompt targets. The string literal `"derive-from-pipeline-composition"` is the policy-atom signal that the runner should derive at startup; deployments that want to explicitly narrow the set write a higher-priority atom with a literal `string[]` of stage names.
...
- `provenance.derived_from`: `string[]` of atom ids that are the SOURCE roots for this re-prompt event. MUST include the pipeline atom id and the source observation atom id (the atom that carried the finding). Source-roots only; this field is NOT used for re-prompt chain parenting.
- `metadata.thread_parent`: `string | null`. The atom id of the IMMEDIATE prior `pipeline-cross-stage-reprompt` atom in the same pipeline, or `null` for the first re-prompt in the chain. The Console reads this field to render the back-and-forth chain; if absent or null, the Console renders the re-prompt as a thread root.

The Console's `/pipelines/<id>` view picks these up via the existing PipelineLifecycle projection and renders a "Deliberation thread" section: each cross-stage re-prompt is a row showing FROM -> TO with the finding message. The Console walks `metadata.thread_parent` to build the chain (root atom has `thread_parent: null`, each subsequent atom points at the immediately prior one). Operator sees the explicit handoff state per their north-star.

The separation between `provenance.derived_from` (source roots, always required) and `metadata.thread_parent` (chain pointer, optional/nullable) keeps the substrate's provenance contract clean: derived_from stays a flat unordered set of taint roots used by the taint propagation engine, while thread_parent is a render-only ordering hint. PR3 emits both; PR5 reads thread_parent for the UI chain and falls back to scanning derived_from only if thread_parent is missing (which signals a malformed atom and is rendered as a thread root with a warning).
