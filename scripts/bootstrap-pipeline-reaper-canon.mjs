#!/usr/bin/env node
/**
 * Canon bootstrap for the pipeline-reaper TTLs.
 *
 * Seeds one L3 policy atom into the .lag/atoms store:
 *
 *   pol-pipeline-reaper-ttls-default   L3 directive: terminal=30d
 *                                                     hil-paused=14d
 *                                                     agent-session=30d
 *
 * Promotes the env-var + CLI-flag knobs that the pipeline reaper module
 * (PR #377) consumes from process configuration to canon configuration
 * per `dev-substrate-not-prescription`: tunable retention thresholds
 * belong in atoms, not constants. The driver script and (post-Task-5)
 * LoopRunner pass keep working as-is; the canon path is an additive
 * higher-priority layer above them.
 *
 * Atom data lives in scripts/lib/pipeline-reaper-canon-policies.mjs so
 ...
 * (same discipline as bootstrap-reaper-canon.mjs and
...
import { buildPolicies, policyAtom } from './lib/pipeline-reaper-canon-policies.mjs';
...
/**
 * Compare a stored pipeline-reaper-policy atom's payload to the
 * expected shape. Returns a list of drift descriptors (empty = in
 * sync). Mirrors diffPolicyAtom from bootstrap-reaper-canon.mjs so the
 * two bootstraps use one drift-check pattern.
 *
...
async function main() {
  ...
  if (DRY_RUN) {
    console.log(
      `[bootstrap-pipeline-reaper-canon] dry-run: ${policies.length} atoms would be written:`,
    );
  ...
  for (const spec of policies) {
    ...
    if (existing === null) {
      ...
      written += 1;
      console.log(`[bootstrap-pipeline-reaper-canon] wrote ${expected.id}`);
      continue;
    }
    const diffs = diffPolicyAtom(existing, expected);
    if (diffs.length > 0) {
      console.error(
        `[bootstrap-pipeline-reaper-canon] DRIFT on ${expected.id}:\n  ${diffs.join('\n  ')}\n`
          + 'Resolve by: (a) editing buildPolicies in '
          + 'scripts/lib/pipeline-reaper-canon-policies.mjs to match stored shape if the '
  ...
  console.log(`[bootstrap-pipeline-reaper-canon] done. ${written} written, ${ok} already in sync.`);
}

main().catch((err) => {
  console.error('[bootstrap-pipeline-reaper-canon] FAILED:', err);
  process.exit(1);
});
