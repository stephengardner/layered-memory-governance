#!/usr/bin/env node
/**
 * Canon bootstrap for the planning-pipeline auditor-feedback
 * re-prompt loop enable + severity-floor dials.
 *
 * Seeds one L3 policy atom into the .lag/atoms store:
 *
 *   pol-auditor-feedback-reprompt-default  L3 directive: max_attempts=2,
 *                                          severities_to_reprompt=['critical']
 *
 * Promotes the runner's hardcoded re-prompt floor to a canon
 * configuration: tunable max attempts + severity-trigger list belongs
 * in atoms, not constants.
 * The runner consumes the policy via
 * readAuditorFeedbackRePromptPolicy(host) and falls through to the
 * hardcoded floor when no atom resolves; this script seeds the atom
 * so the canon path is an additive higher-priority layer.
 *
 * Atom data lives in
 * scripts/lib/auditor-feedback-reprompt-canon-policies.mjs so the test
 ...
 * (same discipline as bootstrap-loop-pass-claim-reaper-canon.mjs and
 * bootstrap-pipeline-reaper-canon.mjs).
 */
...
import { buildPolicies, policyAtom } from './lib/auditor-feedback-reprompt-canon-policies.mjs';
...
if (!OPERATOR_ID) {
  console.error(
    '[bootstrap-auditor-feedback-reprompt-canon] ERROR: LAG_OPERATOR_ID is not set. Export your\n'
...
/**
 * Compare a stored auditor-feedback-reprompt policy atom's payload to
 * the expected shape. Returns a list of drift descriptors (empty = in
 * sync). Mirrors diffPolicyAtom from
 * bootstrap-loop-pass-claim-reaper-canon.mjs so the canon bootstraps
...
async function main() {
  ...
  if (DRY_RUN) {
    console.log(
      `[bootstrap-auditor-feedback-reprompt-canon] dry-run: ${policies.length} atoms would be written:`,
    );
  ...
  for (const spec of policies) {
    ...
    if (existing === null) {
      ...
      written += 1;
      console.log(`[bootstrap-auditor-feedback-reprompt-canon] wrote ${expected.id}`);
      continue;
    }
    const diffs = diffPolicyAtom(existing, expected);
    if (diffs.length > 0) {
      console.error(
        `[bootstrap-auditor-feedback-reprompt-canon] DRIFT on ${expected.id}:\n  ${diffs.join('\n  ')}\n`
          + 'Resolve by: (a) editing buildPolicies in '
          + 'scripts/lib/auditor-feedback-reprompt-canon-policies.mjs to match stored shape if the '
  ...
  console.log(`[bootstrap-auditor-feedback-reprompt-canon] done. ${written} written, ${ok} already in sync.`);
}

main().catch((err) => {
  console.error('[bootstrap-auditor-feedback-reprompt-canon] FAILED:', err);
  process.exit(1);
});
