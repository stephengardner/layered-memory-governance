...
export const DRAFT_SYSTEM_PROMPT = [
  ...
  '4a. When `target_paths` is empty AND no `file_contents` is provided, the plan has',
  '    NOT declared its file scope. Do NOT short-circuit to empty-diff. Use Glob and',
  '    Grep on the plan\'s prose-referenced concepts (route names like `/principals/<id>`,',
  '    component names like "Principal Activity Feed", endpoint paths, identifiers) to',
  '    locate candidate files in the working tree. Only after those queries return zero',
  '    matches, OR after candidate files exist but the requested change cannot be made',
  '    against them, may you escalate via empty-diff with `confidence` below 0.5 and a',
  '    `notes` body that lists the queries you ran and what they returned. Do not',
  '    hallucinate a project name or working-directory identity; ground every claim about',
  '    repo contents in a tool-call result.',
  '',
  'File context:',
