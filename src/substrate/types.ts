...
export type AtomType =
  ...
  // successfully delegates to a deployment-side notifier (e.g. a
  // Telegram bot). Carries provenance.derived_from: [planId] so the
  // audit chain links the push back to its plan, and
  // metadata.plan_id mirrors that pointer for projection-scoped
  // queries. The notify pass refuses to re-notify any plan whose id
  // already appears in this set, which keeps a long-running daemon
  // from spamming the same plan across thousands of ticks.
